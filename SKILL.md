...
- `SKILL_ROOT`：当前 `SKILL.md` 所在目录。只用于读取本 skill 的 `references/` 和运行 `scripts/` 辅助脚本。
- `CONFIG_PATH`：用户实际使用的 `config.json`。它通常在用户工作区，可以不在源码目录里。
- `PROJECT_ROOT`：包含 `pyproject.toml`、`src/` 和可执行同步器的源码目录，或一个已经安装了 `wx-article-sync` CLI 的环境。

执行规则：

1. 配置校验用 `python $SKILL_ROOT/scripts/validate_config.py $CONFIG_PATH`。
2. 状态检查用 `python $SKILL_ROOT/scripts/inspect_state.py <state_path>`。
3. 同步时必须给 `--config` 传入 `CONFIG_PATH` 的绝对路径；只有当当前目录确实包含目标 `config.json` 时，才可以写 `--config config.json`。
4. 如果只有配置工作区，没有源码和虚拟环境，不要把配置复制到源码目录；在 `PROJECT_ROOT` 执行 `uv run wx-article-sync --config /absolute/path/to/config.json`。
5. `output_dir` 和 `state_path` 的相对路径按 `CONFIG_PATH` 所在目录解析；运行日志 `logs/wx-article-sync.log` 按命令执行时的当前目录写入。
...
1. 用 `uv run wx-article-sync --config /absolute/path/to/config.json` 完成本地保存。
...
python scripts/ima_manifest.py /absolute/path/to/data/articles --output /absolute/path/to/data/ima-manifest.jsonl
```

在项目根目录运行同步器，并传入配置文件绝对路径：
