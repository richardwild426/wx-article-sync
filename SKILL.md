...
- 安装完成后的回复要短，只确认能力已可用，并进入下一步配置问题。

安装完成后的回复模板：

```text
wx-article-sync 已安装好。接下来我会带你完成首次配置，先确认一个问题：你想同步几个公众号？
```
...
1. 先问：你想同步几个公众号？
2. 再确认：这些公众号是否已经在 mptext 里添加并同步过文章链接？
3. 再收集每个公众号的 `fakeid`；用户没有时，引导其按 mptext 准备文档获取。
4. 再问同步目标：每天最新几篇，是否需要同步到 ima 知识库。
5. 最后才生成或更新 `config.json`，并运行校验。
...
3. 将 `page_size` 设为 `1`，将 `max_pages` 设为 `1`。
